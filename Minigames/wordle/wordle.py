...
async def _fetch_word() -> str:
    """Fetch a 5-letter word from Wordnik. Falls back to the static list on any error."""
    ...
    try:
        params = {
            ...
            "minLength":           var.WORD_LENGTH,
            "maxLength":           var.WORD_LENGTH,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    if word.isalpha() and len(word) == var.WORD_LENGTH:
                        return word
    except Exception:
        log.warning("Wordle: Wordnik fetch failed — using static word list")
    return random.choice(var.WORDS).upper()
...
def _check_guess(guess: str, word: str) -> list[str]:
    """Return emoji result list for a guess: 🟩 correct spot, 🟨 wrong spot, ⬛ not in word."""
    result    = ['⬛'] * var.WORD_LENGTH
    remaining = list(word)

    for i in range(var.WORD_LENGTH):
        if guess[i] == word[i]:
            result[i]    = '🟩'
            remaining[i] = None
...
def _format_row(guess: str, result: list[str]) -> str:
    return f"{'  '.join(result)}  `{'  '.join(guess)}`"
...
# ============================================================================
# API KEY SETUP  (shared command with hangman — only one cog registers it)
# ============================================================================
...
def _make_set_api_key_cmd():
    @app_commands.command(name="set_api_key", description="Set an external API key used by bot features.")
...
class GuessModal(discord.ui.Modal, title="Make a Guess"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await self._view.process_guess(interaction, self.guess.value.upper())
...
class WordleView(discord.ui.View):
    ...
    def _board_lines(self) -> list[str]:
        lines = [_format_row(g, r) for g, r in self.guesses]
        lines += [_EMPTY_ROW] * (var.MAX_ATTEMPTS - len(self.guesses))
        return lines

    def _build_embed(self) -> discord.Embed:
        attempts  = len(self.guesses)
        remaining = var.MAX_ATTEMPTS - attempts
        embed = discord.Embed(
            title=f"🟩 Wordle — Attempt {attempts + 1}/{var.MAX_ATTEMPTS}",
            color=var.COLOR_PLAYING,
        )
        embed.description = '\n'.join(self._board_lines())
        embed.set_footer(
            text=(
    ...
    async def process_guess(self, interaction: discord.Interaction, guess: str):
        if not guess.isalpha() or len(guess) != var.WORD_LENGTH:
            await interaction.response.send_message(
                f"Enter exactly {var.WORD_LENGTH} letters.", ephemeral=True,
            )
            return

        result = _check_guess(guess, self.word)
        self.guesses.append((guess, result))

        won  = all(e == '🟩' for e in result)
        lost = not won and len(self.guesses) >= var.MAX_ATTEMPTS

        if won or lost:
            await self._end_game(interaction, won=won)
        else:
    ...
    async def _end_game(self, interaction: discord.Interaction, won: bool, gave_up: bool = False):
        ...
        if won:
            mult    = var.WIN_MULTIPLIERS[attempts - 1]
            ...
            embed = discord.Embed(
                title=f"🟩 **{self.word}** — Solved in {attempts}!",
                description=f"Won {sym} **{profit:,}** {var.CURRENCY_NAME}!",
                color=var.COLOR_WIN,
            )
            embed.add_field(name="Attempts",   value=f"{attempts}/{var.MAX_ATTEMPTS}", inline=True)
    ...
    async def on_timeout(self):
        ...
        if self.guesses:
            embed.add_field(name="Board", value='\n'.join(self._board_lines()), inline=False)
        embed.set_footer(text=f"{self.interaction.user.display_name} · {var.SERVER_NAME}")
        try:
...
class WordleCog(commands.Cog):
    ...
    @app_commands.command(
        name="wordle",
        description="Guess the 5-letter word in 6 tries — 🟩 right spot, 🟨 wrong spot, ⬛ not in word.",
    )
    ...
    async def wordle(self, interaction: discord.Interaction, amount: int):
        ...
        view = WordleView(self, interaction, amount, word)
...
async def setup(bot: commands.Bot):
    ...
    log.info("✅ Minigames/Wordle cog loaded")
