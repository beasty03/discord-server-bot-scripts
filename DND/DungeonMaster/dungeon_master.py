...
class DungeonMasterCog(commands.Cog):
    ...
    async def _start_wander(self, interaction: discord.Interaction,
        ...
        if party:
            ...
            join_msg = await interaction.channel.send(embed=join_view._build_embed(), view=join_view)
            asyncio.create_task(
                ...
                if action == "flee":
                    ...
                    if stats:
                        ...
                        if crit or total >= enemy["ac"]:
                            ...
                            round_lines.append(
                                f"⚔️ **{name}** rolled {roll} ({total:+d} vs AC {enemy['ac']}) → "
                            ...
                            round_lines.append(
                                f"⚔️ **{name}** rolled {roll} ({total:+d} vs AC {enemy['ac']}) → MISS")
                            run["log"].append({
                                "type": "attack", "uid": uid, "name": name,
                ...
                if non_fled:
                    ...
                    if stats:
                        ...
                        if roll == 20 or total_atk >= dodge_ac:
                            ...
                            round_lines.append(
                                f"💥 **{enemy['name']}** hits **{target_name}** → `{dmg}` dmg{crit_txt}{dodge_txt}")
                            run["log"].append({
